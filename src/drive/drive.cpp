...
void Drive::tank_control() {
   //Grabs joystick position
   int lvert = master.get_analog(ANALOG_LEFT_Y);
   int rvert = master.get_analog(ANALOG_RIGHT_Y);
   //Checks to make sure joystick isn't it deadzone
   if (abs(lvert) < deadzone){
       lvert = 0;}
   if (abs(rvert) < deadzone){
      rvert = 0;}
   lvert = lvert * 100;
   rvert = rvert * 100;
   if (master.get_digital(DIGITAL_A)){
   ...
   DriveL.move_voltage(lvert);
   DriveR.move_voltage(rvert);
}
...
float Drive::get_absolute_heading(){
  //float heading = Gyro.get_heading() * (360/gyro_scale);
